import { ZkRSSContractArtifact, ZkRSSContract } from "../../artifacts/ZkRSS.js"
...
describe("Accounts", () => {
    ...
    it("Creates accounts with fee juice", async () => {
        ...
        console.log('Deploying first ZkRSSContract to progress blocks...');
        await ZkRSSContract.deploy(wallet).send({
            from: ownerAccount.address,
            fee: { paymentMethod: sponsoredPaymentMethod }
        }).deployed({ timeout: getTimeouts().deployTimeout }); // deploy contract with first funded wallet
        console.log('First ZkRSSContract deployed');

        console.log('Deploying second ZkRSSContract to progress blocks...');
        await ZkRSSContract.deploy(wallet).send({
            from: ownerAccount.address,
            fee: { paymentMethod: sponsoredPaymentMethod }
        }).deployed({ timeout: getTimeouts().deployTimeout }); // deploy contract with first funded wallet
        console.log('Second ZkRSSContract deployed');
    ...
    it("Sponsored contract deployment", async () => {
        ...
        const ContractArtifact = ZkRSSContractArtifact
        ...
        const deploymentData = await getContractInstanceFromInstantiationParams(ContractArtifact,
            {
                constructorArgs: [],
                salt,
                deployer: deployerAccount.getAddress()
            });
        const deployer = new ContractDeployer(ContractArtifact, wallet);
